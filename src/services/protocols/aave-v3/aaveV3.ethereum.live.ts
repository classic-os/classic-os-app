// src/services/protocols/aave-v3/aaveV3.ethereum.live.ts
        ...
        try {
            ...
            if (reserves.length > 0) {
                ...
                const reserveDataMap = new Map<
                    Address,
                    { currentATokenBalance: bigint; currentStableDebt: bigint; currentVariableDebt: bigint }
                >();
                ...
                if (assetsToFetchMetadata.length > 0) {
                    ...
                    const metadataCalls = assets.flatMap((assetAddr) => [
                        { address: assetAddr, abi: ERC20_ABI, functionName: "decimals" as const, args: [] as const },
                        { address: assetAddr, abi: ERC20_ABI, functionName: "symbol" as const, args: [] as const },
                        { address: assetAddr, abi: ERC20_ABI, functionName: "name" as const, args: [] as const },
                    ]);
                    ...
                    for (let i = 0; i < assets.length; i++) {
                        ...
                        const decimals =
                            decimalsResult?.status === "success" ? toNumberU8(decimalsResult.result, 18) : 18;
                ...
                reserveDataMap.forEach((data, assetAddr) => {
                    ...
                    if (data.currentATokenBalance > 0n) {
                        supplied.push({ token, raw: data.currentATokenBalance });
                    }

                    const totalBorrow = data.currentStableDebt + data.currentVariableDebt;
                    if (totalBorrow > 0n) {
                        borrowed.push({ token, raw: totalBorrow });
                    }
                });
            }
        } catch (error) {
            console.warn("Failed to fetch reserve-level positions, falling back to empty", error);
        }
